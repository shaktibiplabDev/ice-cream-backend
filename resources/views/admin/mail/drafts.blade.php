{{-- resources/views/admin/mail/drafts.blade.php --}}
    ...
    <div class="inbox-main-layout">
        ...
        <aside class="inbox-sidebar">
            <nav class="sidebar-nav">
                ...
                <a href="{{ route('admin.mail.drafts') }}" class="nav-link active">
                    <svg class="nav-icon" width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
        ...
        <main class="inbox-email-list">
            @if($emails->count() > 0)
                <div class="email-table-wrapper">
                    <div class="email-table-header">
                        ...
                        <div class="header-date">Saved</div>
                        <div class="header-actions"></div>
                    </div>

                    <div class="email-rows">
                        @foreach($emails as $email)
                            <div class="email-row draft-row">
                                <div class="row-to">
                                    <span class="sender-name draft-recipient">
                                        {{ $email->to_email ?? 'No recipient' }}
                                    </span>
                                </div>
                                <div class="row-subject">
                                    <span class="email-subject draft-label">Draft</span>
                                ...
                                <div class="row-actions">
                                    <a href="{{ route('admin.mail.compose') }}?draft={{ $email->id }}"
                                       class="action-btn action-edit" title="Edit draft">
                                    ...
                                    <form action="{{ route('admin.mail.destroy', $email) }}" method="POST"
                                          style="display:inline" onsubmit="return confirm('Delete this draft?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="action-btn action-delete" title="Delete draft">
                                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <div class="inbox-pagination">
                    <span class="pagination-info">
                        Showing {{ $emails->firstItem() }} to {{ $emails->lastItem() }} of {{ $emails->total() }} drafts
                    </span>
            ...
            @else
                <div class="empty-inbox">
                    ...
                    <h3>No drafts</h3>
                    <p>Emails you save as drafts will appear here.</p>
...
<style>
    ...
    .header-date, .row-date   { width: 60px; flex-shrink: 0; text-align: right; font-size: 12px; color: var(--text-muted, #8e8e96); }
    ...
    .draft-recipient { color: var(--text-muted, #8e8e96); font-style: italic; }
    ...
    .action-edit:hover { background: rgba(79,70,229,.25); }
    .action-delete {
